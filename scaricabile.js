...
function searchJobs(jobs, title, location) {
  //cerco anche sa sono mauscole o minuscole
  let lowerPosition = title.toLowerCase();
  ...
  for (let i = 0; i < jobs.length; i++) {
    let job = jobs[i];
    let lowerTitle = job.title.toLowerCase();
    let lowerJobLocation = job.location.toLowerCase();

   
    if (lowerTitle.includes(lowerPosition) && lowerJobLocation.includes(lowerLocation)) {
      //estraggo title e location da job
      let { title, location } = job;
      results.push({ title, location });
      //aumento il numero di count
      count++;
    }
  }
